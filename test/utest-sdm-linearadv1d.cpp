...
//  gp.send( wave_speed_field.coordinates().array() , wave_speed_field.array() );
  ...
  Field& rank = solution_field.dict().create_field("rankfield",1);
  Field& rank_sync = solution_field.dict().create_field("rank_sync",1);
...
//#endif
  ...
  Field& rank = solution_field.dict().create_field("elem_rank",1);
  Field& rank_sync = solution_field.dict().create_field("rank_sync",1);
  for (Uint r=0; r<rank.size(); ++r)
  {
    rank[r][0] = rank.rank()[r];
