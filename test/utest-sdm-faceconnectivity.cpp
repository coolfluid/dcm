...
BOOST_AUTO_TEST_CASE( test_P2_quad )
{
  ...
  boost::shared_ptr< QuadLagrange1D<2> > sf = allocate_component< QuadLagrange1D<2> >("sf");
...
BOOST_AUTO_TEST_CASE( test_P2_hexa )
{
  ...
  enum CellOrientation {MATCHED=0, INVERTED=1};
  boost::shared_ptr< HexaLagrange1D<2> > sf = allocate_component< HexaLagrange1D<2> >("sf");
