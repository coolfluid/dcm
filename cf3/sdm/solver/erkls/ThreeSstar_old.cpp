...
#include "cf3/common/EventHandler.hpp"
...
#include "cf3/sdm/solver/erkls/RungeKuttaLowStorage3.hpp"
#include "cf3/sdm/solver/TimeIntegrationStepComputer.hpp"
...
common::ComponentBuilder < RungeKuttaLowStorage3, common::Action, LibERKLS > RungeKuttaLowStorage3_Builder;

///////////////////////////////////////////////////////////////////////////////////////

RungeKuttaLowStorage3::RungeKuttaLowStorage3 ( const std::string& name ) :
  sdm::solver::Solver(name)
{
...
//  options().add("domain_discretization",m_domain_discretization).link_to(&m_domain_discretization);
  ...
  options()["solution"].attach_trigger( boost::bind( &RungeKuttaLowStorage3::create_solution_backup , this) );
...
void RungeKuttaLowStorage3::create_solution_backup()
{
...
void RungeKuttaLowStorage3::step()
{
  ...
  for (Uint stage=0; stage<nb_stages; ++stage)
  {
    ...
    if ( is_not_null( m_pre_update ) ) pre_update()->execute();
...
//      m_domain_discretization->execute();
    ...
    if ( is_not_null( m_post_update ) ) post_update()->execute();
