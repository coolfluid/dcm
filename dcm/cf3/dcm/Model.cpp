...
common::ComponentBuilder < Model, common::Component, LibDCM > Model_Builder;
...
//  m_residual_norm_computer = m_tools->create_static_component<dcm::ComputeLNorm>("compute_residual_norm");
...
{
  common::XML::SignalOptions opts(args);
  Handle<solver::PDESolver> solver = add_solver( opts.value<std::string>("name"),
                                                 opts.value<Handle<solver::PDE> >("pde"),
...
{
  common::XML::SignalOptions opts(args);
  opts.add("name",std::string("RK_solver"));
  opts.add("pde",Handle<solver::PDE>());
  opts.add("solver",std::string("cf3.dcm.solver.erkls.TwoSstar"));
...
Handle<solver::PDESolver> Model::add_solver( const std::string& name,
                                             const Handle<solver::PDE>& pde,
                                             const std::string& solver,
                                             const std::string& time_step_computer )
{
  Handle<solver::PDE> _pde = pde;
  if( is_null(_pde) )
    _pde = find_component_ptr<solver::PDE>(*this);
  if( is_null(_pde) )
    throw SetupError( FromHere(), "pde is not valid" );
  Handle<solver::PDESolver> pde_solver = m_time_stepping->create_component(name,solver)->handle<solver::PDESolver>();
  pde_solver->mark_basic();
  pde_solver->options().set("pde",_pde);
  pde_solver->options().set("time_step_computer",time_step_computer);
  m_time_stepping->add_time(_pde->time());
  return pde_solver;
}
