...
#include "cf3/dcm/equations/advectiondiffusion/AdvectionDiffusion1D.hpp"
...
BOOST_AUTO_TEST_CASE( test_advection_1d )
{
  ...
  Handle<dcm::Model> model = Core::instance().root().create_component<dcm::Model>("adv1d");
  ...
  // Handle<solver::BC> bc_mirror = pde->add_bc("mirror","cf3.dcm.equations.advectiondiffusion.BCMirror1D",bc_regions);
  ...
  Handle<solver::PDESolver> solver = model->add_solver( "rk_solver", pde,
...
{
  // Create simulation model
  Handle<dcm::Model> model = Core::instance().root().create_component<dcm::Model>("diff1d");
...
{
  // Create simulation model
  Handle<dcm::Model> model = Core::instance().root().create_component<dcm::Model>("adv2d");
...
{
  // Create simulation model
  Handle<dcm::Model> model = Core::instance().root().create_component<dcm::Model>("diff2d");
